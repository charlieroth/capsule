...
use crate::fetcher::types::{PageResponse, Charset};
...
async fn test_extract_article() {
    ...
    assert!(content.html.contains("https://example.com/images/sample.jpg"));
}
...
async fn test_malformed_html() {
    let html = "<html><head><title>Broken</title><body><p>Unclosed tags<div>More content".to_string();
